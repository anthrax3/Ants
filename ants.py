...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(self.max_range):
            if current == hive:
                return None
            if self.min_range <= i and len(current.bees) != 0:
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.ant != None:
            self.ant.action(colony)
        another_list = list(self.place.bees)
        for bee in another_list:
            bee.reduce_armor(self.damage)
        # END Problem 12
    ...
    def action(self, colony):
        ...
        if self.real == False:
            ...
            while check_place != None:
                if check_place.ant != None:
                    if check_place.ant not in self.doubled:
                        check_place.ant.damage = check_place.ant.damage * 2
                        self.doubled.append(check_place.ant)
                    if check_place.ant.container == True and check_place.ant.ant not in self.doubled and check_place.ant.ant != None:
                        check_place.ant.ant.damage = check_place.ant.ant.damage * 2
                        self.doubled.append(check_place.ant.ant)
